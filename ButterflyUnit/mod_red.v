...
module mod_red(
    ...
    carry_save_adder add0(.ip1(add_ip1),.ip2(add_ip2),.ip3(add_ip3),.ip4(add_ip4),.ip5(add_ip5),.ip6(add_ip6),.ip7(add_ip7),.ip8(add_ip8),.s(sum1),.c(carr1),.det(det));
    ripple_carry_adder add1(.x(sum1),.y(carr1),.z(temp1));
    reg_30b rf(.ip(temp1),.clk(clk),.rst(rst),.op(temp2));
    selector s0(.x(temp2),.dk(dk),.y(c),.clk(clk),.det(det));
endmodule
